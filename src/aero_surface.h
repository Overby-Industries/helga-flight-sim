...
//
// Control response is a weighted mix of all three pilot inputs
// (elevator_gain/aileron_gain/rudder_gain), not a single exclusive
// axis -- because Helga's actual flight controls (per
// overbyindustries.space/aerospace/flight-controls) are elevons that
// combine pitch and roll on the same trailing-edge surface, plus a
// split rudder for yaw, not a conventional separate tailplane. A left
// elevon sets elevator_gain=1, aileron_gain=+1; the right elevon sets
// elevator_gain=1, aileron_gain=-1, so a pure elevator input deflects
// both the same way and a pure aileron input deflects them oppositely,
// exactly like a real elevon pair.
